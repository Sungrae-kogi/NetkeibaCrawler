...
def upload_all_csv_to_db(target_date=None, target_venue=None):
    project_root = str(BASE_DIR.parent.parent.parent)
    if project_root not in sys.path:
        sys.path.append(project_root)
    from src.crawlers.WebCrawler.discovery import get_all_target_races
    ...
    for t in targets_info:
        ...
        d = t['date']
        TARGETS.extend([
            (BASE_DIR.parent.parent / "crawlers" / "WebCrawler" / "entry_sheet_2" / "data" / f"api_entry_sheet_2_{v}_{d}.csv", "tmp_races"),
            (BASE_DIR.parent.parent / "crawlers" / "TRNOCrwaler" / "data" / f"TRNO_result_{v}_{d}.csv", "tmp_trainers"),
            (BASE_DIR.parent.parent / "crawlers" / "HRNOCrawler" / "data" / f"HRNO_result_{v}_{d}.csv", "tmp_horses"),
            (BASE_DIR.parent.parent / "crawlers" / "JKNOCrawler" / "data" / f"JKNO_result_{v}_{d}.csv", "tmp_jockeys")
        ])
