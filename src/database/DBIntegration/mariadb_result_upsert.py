...
LOG_FILE = LOG_DIR / f"{date_str}_DBUpsert.log"
...
def upload_result_csv_to_db(target_date, target_venue):
    ...
    TARGETS = [
        (BASE_DIR.parent.parent / "crawlers" / "WebCrawler" / "data" / f"race_planning_{target_venue}_{target_date}.csv", "tmp_races"),
        (BASE_DIR.parent.parent / "crawlers" / "TRNOCrwaler" / "data" / f"TRNO_result_{target_venue}_{target_date}.csv", "tmp_trainers"),
        (BASE_DIR.parent.parent / "crawlers" / "HRNOCrawler" / "data" / f"HRNO_result_{target_venue}_{target_date}.csv", "tmp_horses"),
        (BASE_DIR.parent.parent / "crawlers" / "JKNOCrawler" / "data" / f"JKNO_result_{target_venue}_{target_date}.csv", "tmp_jockeys")
    ]
