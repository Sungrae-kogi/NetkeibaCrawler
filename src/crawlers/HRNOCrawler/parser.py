...
def sanitize_text(val):
    if isinstance(val, str):
        # 줄바꿈/탭을 공백으로 치환 후, 제어문자 삭제 및 양끝 공백 제거
        text = re.sub(r'[\r\n\t]+', ' ', val)
...
# ==========================================
# 2. 비동기 통신 및 VPN 방어 함수들
# ==========================================
import subprocess

vpn_lock = asyncio.Lock()

async def reconnect_vpn_async():
    if vpn_lock.locked():
        # 이미 다른 태스크가 VPN 재연결 중이면 끝날 때까지 대기만 함
        async with vpn_lock:
            pass
        return
    ...
    async with vpn_lock:
        try:
            ...
            print("🌐 [NordVPN] VPN 연결 해제 중...")
            await asyncio.to_thread(subprocess.run, [nordvpn_path, "-d"], shell=False, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
            await asyncio.sleep(3)
            print("🌐 [NordVPN] VPN 재연결 중 (Japan 서버)...")
            await asyncio.to_thread(subprocess.run, [nordvpn_path, "-c", "-g", "Japan"], shell=False, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
            print("🌐 [NordVPN] 재연결 완료! 안정화를 위해 10초 대기...")
            await asyncio.sleep(10)
        except Exception as e:
            print(f"🌐 [NordVPN] 제어 중 예외 발생: {e}")
...
async def fetch_with_retry(
    ...
    """네트워크 오류 시 VPN 재연결 방어로직을 포함한 비동기 fetch 함수"""
    for i in range(retries):
        try:
            async with session.get(url, params=params, headers=HEADERS, timeout=20) as res:
                ...
                elif res.status in [403, 429, 500, 502, 503, 504]:
                    print(f"⚠️ 접속 차단 또는 서버 에러(상태코드 {res.status}). VPN 재연결 시도 ({i+1}/{retries})")
                    await reconnect_vpn_async()
                    continue
                else:
                    return None
        except (aiohttp.ClientError, asyncio.TimeoutError) as e:
            print(f"⚠️ 타임아웃 또는 네트워크 에러 발생: {e}. VPN 재연결 시도 ({i+1}/{retries})")
            await reconnect_vpn_async()
            if i == retries - 1:
                raise e
...
async def parse_horse_page(
    ...
    # ✨ 최종 반환 직전, 딕셔너리의 모든 값을 세탁기에 돌립니다.
    cleaned_out = {k: sanitize_text(v) for k, v in out.items()}
    return cleaned_out